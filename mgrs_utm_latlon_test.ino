...
void setup() {
  ...
  for (int i = 0; i < 500; i++) {
    ...
    retval = MGRSToUTM(mgrs, &Zone, &Hemisphere, &Easting, &Northing);
    if (retval == MGRS_NO_ERROR) {
      ...
      UTMToLL (Easting, Northing, Zone, Hemisphere == 'N', lat, lon);
...
long MGRSToUTM (char   *MGRS,
                          long   * Zone,
                          char   *Hemisphere,
                          long * Easting,
                          long * Northing)
/*
   The function MGRSToUTM converts an MGRS coordinate string
...
int UTMToLL(long easting, long northing, int zone, bool isNorthernHemisphere, double & lat, double & lon) {
  double a = 6378137.0;
